...
def common_data(i, incident):
    ...
    assert i.timestamp == datetime.datetime(2015, 3, 30, 21, 10, 38), "timestamp is: %r" % incident.timestamp # should be in UTC
    ...
    assert i.host == "somehostname.at", "unexpected host, was: %r" % incident.host
    assert i.path == "/fubar/sr/10/SomeAction.do", "invalid path, was:%r" %incident.path
